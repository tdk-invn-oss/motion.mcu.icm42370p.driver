...
/*
 * This example showcases how to configure IMU to stream accel data.
 */
...
int main(void)
{
	...
	INV_MSG(INV_MSG_LEVEL_INFO, "### Example Accel");
	INV_MSG(INV_MSG_LEVEL_INFO, "###");
...
static void sensor_event_cb(inv_imu_sensor_event_t *event)
{
	...
	if (print_si) {
		...
		INV_MSG(INV_MSG_LEVEL_INFO, "SI  %10llu us   %s   %s   %s", int_timestamp, accel_str,
		        temp_str, fifo_time_str);
	}
	...
	if (print_lsb) {
		...
		INV_MSG(INV_MSG_LEVEL_INFO, "LSB %10llu us   %s       %s        %s", int_timestamp,
		        accel_str, temp_str, fifo_time_str);
	}
}
